...
def parse_args():
    ...
    return args.db_name, args.molecular_database, args.index_file, args.top_k, args.threshold, args.num_workers, args.output_file, args.query_path, args.on_disk, args.hac_limits, args.mw_range, args.search_type
...
def similarity_search(
    ...
    fpe = FPSim2Engine(db_name, in_memory_fps=False if on_disk else True)
    search, tanimoto = {}, {}
...
def main():
    db_name, molecular_database, index_file, top_k, threshold, num_workers, output_file, query_path, on_disk,hac_limits, mw_range, search_type = parse_args()
    ...
    if search_type == "similarity":    
        search_results, tanimoto = similarity_search(db_name, query, top_k, threshold, num_workers, on_disk)
    search = RetrieveSmiles(index_file, molecular_database, mw_range, hac_limits)
    smiles = search.run(search_results, more_data=tanimoto)
