...
class FPSim2Query:
    ...
    def similarity_search(
        ...
        for que in self.queries:
            results = method(que, k=top_k, threshold=threshold, n_workers=self.workers)
...
def read_search_results(top_k: int = 100, 
    ...
    if search_type == "similarity":
        return {query: sorted(item, key=lambda x: x[1], reverse=True)[:top_k] for query, item in search_results.items()}
...
def main():
    ...
    match stage:    
        case "search":
            process_query_by_db(db_name, query, num_workers, on_disk, top_k, threshold, search_type, outpath=outpath)
